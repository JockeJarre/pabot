...
--argumenttemplate [FILEPATH]
  Instead of using the "--argumentfile[0-n] file" parameter, setting 
  --argumenttemplate templatefileinput and --argumentdata csvfileinput
  the argument files are generated by replacing the columns of the csvfiles into the template.
  Then the same suite runs with the argumentfiles generated.
  For example "--argumenttemplate template.txt --argumentdata templatedata.csv".

--argumentdata [CSV FILEPATH]
  Only to be used with argumenttemplate parameter. The CSV file is read and
  generates template files using the argumenttemplate file.
...
def _parse_args(args):
    ...
    while args and (args[0] in ['--' + param for param in ['command',
        ...
        if args[0] == '--suitesfrom':
            ...
            args = args[2:]
        if args[0] == '--argumenttemplate':
            pabot_args['argumenttemplate'] = args[1]
            args = args[2:]
        if args[0] == '--argumentdata':
            pabot_args['argumentdata'] = args[1]
            args = args[2:]
...
def main(args):
    ...
    try:
        ...
        _check_argumentdata(outs_dir, datasources, options, pabot_args)
